...
- **MCP Protocol Security 详细控制** *(v3.2 新增, integrated from skill-audit KB)*
  - **MCP01 工具描述投毒防护**: 工具 description 字段不得包含指令性内容，LLM 消费前需净化
  - **MCP02 跨源工具隔离**: 不同 MCP Server 的工具执行上下文严格隔离，禁止未授权跨 Server 数据访问
  - **MCP03 工具影子防护**: 检测重复工具名注册，强制 Server 级命名空间，防止合法工具被覆盖
  - **MCP04 工具参数注入防护**: 所有工具参数需 Schema 验证，防止命令注入/路径遍历/SQL注入
  - **MCP05 Server 身份验证**: 连接前验证 MCP Server 身份（证书/签名），防止 Server 冒充
  - **MCP06 工具输出间接注入防护**: 工具返回值在 LLM 消费前需净化，防止通过工具输出注入指令
  - **MCP07 工具资源耗尽防护**: 每工具设执行超时、调用频率限制、输出大小限制
  - **MCP08 能力声明验证**: 运行时验证 MCP Server 实际行为是否匹配其声明的能力
  - **参考威胁**: agentic-threats.yaml `owasp_mcp_threats` 节 (MCP01-MCP08)
  - **CWE**: CWE-1336, CWE-77, CWE-78, CWE-22, CWE-89, CWE-290, CWE-400
...
- **Embedding/Prompt Injection 语义检测** *(v3.2 新增, integrated from skill-audit KB)*
...
- **Evasion & Obfuscation 检测** *(v3.2 新增, integrated from skill-audit KB)*

...
- 增加了 Claude Code / Codex 及 agentskills.io 规范的多 Agent 平台兼容性
- 双环境路径解析（`${SKILL_PATH:-$CLAUDE_SKILL_DIR}`），覆盖 SKILL.md 及全部 9 个 Phase 文件
- 新增 `agents/openai.yaml` 用于 Codex CLI UI 集成
- 升级 `skill_path.sh`：F-09 目录劫持防御 + POSIX 兼容性
- agentskills.io 不变量合规：8/9 C-invariant 通过（C5 代码围栏为已接受风险）
- 全面代码质量审计，对齐 Skill 设计原则
- 完整覆盖 OWASP MCP Top 10 (2025)，采用官方 MCP01:2025-MCP10:2025 编号体系
- SAO 行为模型及 13 条预置 Agent 攻击链（含 MITRE ATT&CK 技术序列）
